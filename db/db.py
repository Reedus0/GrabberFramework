...
class DB():
    ...
    def __init__(self, host, port, user, password, database) -> None:
        ...
        while (1):
            try:
                self.__connection = psycopg2.connect(dbname=database, user=user,
    ...
    def sampleExists(self, sample: dict):
        cursor = self.__connection.cursor()
    ...
    def addSample(self, sample: dict) -> None:
        address_ids = []
        domain_ids = []
        ...
        try:
            cursor.execute("""
                ...
                RETURNING id
                """, (sample["sha256_hash"][:64], sample["md5_hash"][:32] if sample["md5_hash"] else None,
                      sample["malware_family"][:32] if sample["malware_family"] else None,
                      sample["mime"][:32] if sample["mime"] else None, sample["first_seen"],
                      sample["last_seen"], sample["file_name"][:64],
        ...
        for id in address_ids:
            try:
                cursor.execute(
                    "INSERT INTO sample_ip_address_relation(sample_id, ip_address_id) VALUES (%s, %s)", (sample_id, id,))
                self.__connection.commit()
            except (psycopg2.errors.UniqueViolation):
                pass
            self.__connection.commit()
